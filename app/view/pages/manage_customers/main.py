...
from .customer_input_form import CustomerInputForm
...
class ManageCustomers(CTkFrame):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.data = self.__context['customers']

        CTkLabel(
            master=self, text="مدیریت مشتریان", font=("B Koodak Bold", 25)
        ).pack(fill="x", anchor="center", pady=10)

        self.__columns = [('name', 'نام'), ('last_name', 'نام خانوادگی')]

        def show_input_modal():
            self.__input_window = CustomerInputForm(self.__add_callback)
    ...
    def __add_callback(self, name, last_name):
        self.__context.controller.add_customer(name, last_name)

    def __delete_callback(self, customer_id: int):
        def delete(confirmed: bool):
            if confirmed:
                self.__context.controller.remove_customer(customer_id)

        self.__delete_window = CustomerDelete(delete)
        self.__delete_window.grab_set()

    def __update_callback(self, customer: dict):
        def update(data):
            self.__context.controller.update_customer(customer['id'], data['name'], data['last_name'])

        self.__update_window = CustomerUpdateForm(customer, update)
