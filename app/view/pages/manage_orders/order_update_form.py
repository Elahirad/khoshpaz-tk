...
class OrderUpdateForm(CTkToplevel):
    def __init__(self, current_values: dict, callback: Callable, master=None, *args, **kwargs) -> None:
        ...
        CTkLabel(
            master=self, text="ویرایش سفارش", font=("B Koodak Bold", 25)
        ...
        self.__customer_combobox_var = StringVar(
            value=self.__customer_names[
                next(i for i, v in enumerate(self.__customers) if v['id'] == self.__current_values['customer_id'])])
        ...
        self.__paid_amount_entry.insert(0, self.__current_values['paid_amount'])
        ...
        self.__prep_time_entry.insert(0, self.__current_values['preparation_time'])
        ...
        self.__selected_date = datetime.datetime.strptime(self.__current_values['accept_time'], '%Y-%m-%d')
        ...
        self.__selected_foods = self.__current_values['foods_raw']
